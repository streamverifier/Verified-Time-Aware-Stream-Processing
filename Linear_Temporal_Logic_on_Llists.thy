...
lemma HLD_Stream[simp]: "HLDLL X (LCons x \<omega>) \<longleftrightarrow> x \<in> X"
...
primrec lshift :: "'a list \<Rightarrow> 'a llist \<Rightarrow> 'a llist" (infixr \<open>@@-\<close> 65) where
  "lshift [] s = s"
| "lshift (x # xs) s = LCons x (lshift xs s)"
...
lemma ldrop_shift: "ldrop i (w @@- s) = drop i w @@- ldrop (i - length w) s"
  ...
  by (metis (no_types, lifting) Cons_nth_drop_Suc Suc_pred diff_Suc_Suc diff_zero drop.simps(1) drop0 drop_Suc eSuc_enat ldrop_simps(3) length_drop length_greater_0_conv list.size(3) lshift.simps(1) lshift.simps(2))
...
lemma shift_eq_shift_ldropn_length:
  ...
  subgoal for a xs lxs lys ys
    apply (cases ys)
     apply auto[1]
    apply (metis length_greater_0_conv list.size(3) lshift.simps(1))
    apply simp
    done
  done
...
using assms by induct (metis lshift.simps(1), metis lshift.simps(2))+
